...
def plotly_plot_sales(sales, products):
    ...
    """
    df_sales = pd.DataFrame(list(sales.values())) 
    ...
    fig1, dataframe_company_products = drawSalesEvolution(df_sales, products)
    fig2, dataframe_company_allstorages = drawSalesDistribution(df_sales, products)
...
def plotly_plot_stocks(inventory, products):
    ...
    This function display plotly plots
    """
    df_inventory = pd.DataFrame(list(inventory.values()))
    ...
    fig1, general, nord, sud, ouest = drawStocks(df_inventory, products)
